    ...
    <section id="clients" className="relative bg-[#fbf9f4] py-32 overflow-hidden border-t border-b border-[#ede8df]">
      ...
      <div className="relative w-full group">
        ...
        <div className="relative bg-white/40 backdrop-blur-2xl border-y border-[#c8973a]/20 py-16 shadow-[0_20px_50px_rgba(0,0,0,0.02)]">
          ...
          <div className="marquee-inner flex gap-32 items-center animate-marquee-slow">
            {duplicatedBrands.map((brand, i) => (
              ...
              >
                {/* Logo Container */}
                <div
                  className={`relative w-40 h-24 flex items-center justify-center ${desktopBadgeClass(brand.accentBadge)}`}
                >
                  <img 
                    src={brand.src} 
                    alt={brand.name}
      ...
      <div className="md:hidden grid grid-cols-2 gap-6 px-6 mt-16 relative z-10">
        {brands.map((brand, i) => (
          <div key={i} className="flex flex-col items-center p-10 bg-white/60 backdrop-blur-xl border border-[#c8973a]/10 rounded-3xl shadow-sm">
           <div className={`w-24 h-16 flex items-center justify-center ${mobileBadgeClass(brand.accentBadge)}`}>
                <img src={brand.src} alt={brand.name} className="max-w-full max-h-full object-contain filter drop-shadow-md" />
             </div>
