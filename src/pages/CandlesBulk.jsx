...
export default function CandlesBulk() {
  ...
  useEffect(() => {
    document.title = 'Bulk Candles | Amor Exotic';
  ...
  return (
    <Layout>
          ...
          <div className="max-w-3xl">
            ...
            <div className="flex flex-wrap items-center gap-10 mt-14 reveal-up" style={{ animationDelay: '0.3s' }}>
              ...
              <div className="flex flex-col gap-1">
                <span className="text-gold-400 font-serif text-2xl tracking-normal">500K+</span>
      ...
      <section className="py-16 sm:py-24 px-5 sm:px-8 relative overflow-hidden bg-[#f3eee2] border-t border-[#d8cfbe] border-b border-[#dfd5c3]">
        <div className="abs-orb" style={{ top: '-20%', left: '-10%', width: '500px', height: '500px', background: 'rgba(212,168,66,0.10)' }}></div>
        <div className="abs-orb" style={{ bottom: '-30%', right: '-10%', width: '600px', height: '600px', background: 'rgba(212,168,66,0.06)' }}></div>

        <div className="max-w-screen-xl mx-auto relative z-10 text-center mb-12 reveal-up">
          <div className="flex items-center justify-center gap-4 mb-5">
            <div className="h-px w-12 bg-gold-400/40"></div>
            <p className="text-[10px] tracking-[0.6em] uppercase text-gold-600 font-semibold">Corporate Gifting</p>
            <div className="h-px w-12 bg-gold-400/40"></div>
          </div>
          <h2 className="font-serif text-4xl sm:text-5xl font-light text-[#1f1a14] leading-tight">
            Trusted by <span className="italic text-gold-600">Leading Partners</span>
          </h2>
          <p className="text-[11px] tracking-[0.2em] uppercase mt-4 text-[#1f1a14]/50">
            Brands we have served for corporate gifting and premium fragrance experiences
          </p>
        </div>

        <div className="relative max-w-screen-xl mx-auto reveal-up" style={{ animationDelay: '0.1s' }}>
          <div className="absolute left-0 top-0 bottom-0 w-20 bg-gradient-to-r from-[#f3eee2] to-transparent z-20 pointer-events-none"></div>
          <div className="absolute right-0 top-0 bottom-0 w-20 bg-gradient-to-l from-[#f3eee2] to-transparent z-20 pointer-events-none"></div>

          <div className="py-8 border-y border-[#c9b696]/35 bg-white/35 backdrop-blur-sm overflow-hidden">
            <div className="marquee-inner flex gap-16 sm:gap-24 items-center animate-marquee-slow">
              {giftingClientsLoop.map((brand, i) => (
                <div key={i} className="shrink-0 w-44 h-20 flex items-center justify-center px-4 rounded-xl border border-white/40 bg-white/65 shadow-[0_10px_20px_rgba(40,30,20,0.06)]">
                  <div className={`w-full h-full flex items-center justify-center ${brand.logoBg === 'dark' ? 'rounded-lg bg-[linear-gradient(135deg,#1f2a44,#2f3f6a)] border border-white/10 px-3' : ''}`}>
                    <img src={brand.src} alt={brand.name} className="max-w-full max-h-full object-contain" loading="lazy" decoding="async" />
                  </div>
                </div>
              ))}
            </div>
            ...
            {products.map((p, i) => (
              <div key={i} className="product-card group/card reveal-up bg-white border border-black/5 hover:border-gold-400/20 transition-all duration-700 shadow-sm hover:shadow-2xl rounded-px overflow-hidden" style={{ animationDelay: p.delay }}>
                ...
                <div className="p-12">
                  ...
                  <a href={createWhatsAppLink(p.waText, { isEncoded: true })}
        ...
        <div className="max-w-screen-xl mx-auto px-5 sm:px-8 relative z-10">
          <div className="flex flex-col lg:flex-row items-center gap-32">
            <div className="flex-1 reveal-up">
              <h2 className="font-serif text-5xl md:text-8xl text-white font-light leading-tight mb-12">Soothe Your <br /><span className="italic gold-shimmer-text">Space & Mind</span></h2>
              <div className="space-y-12">
                {[
                  { title: 'Deep Relaxation', desc: 'Soft candle fragrance helps reduce stress and creates a calm, comforting atmosphere.' },
                  { title: 'Fresh Environment', desc: 'Balanced scent diffusion keeps rooms feeling fresh, clean, and welcoming throughout the day.' },
                  { title: 'Mind Refresh', desc: 'Designed for daily reset rituals that improve focus, mood, and overall mental clarity.' }
                ].map((f, i) => (
            ...
            <div className="flex-1 reveal-up" style={{ animationDelay: '0.4s' }}>
              <div className="relative aspect-[4/5] rounded-[30px] border border-gold-400/10 overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)] group">
                ...
                <div className="absolute bottom-12 left-12 right-12 p-8 border border-white/10 backdrop-blur-xl bg-black/40 rounded-2xl shadow-2xl">
                  ...
                  <p className="text-white/80 text-xs leading-relaxed italic">"A gentle candle ambience that relaxes the room, refreshes the air, and clears the mind."</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="relative p-8 sm:p-16 md:p-24 rounded-[40px] border border-white/60 shadow-[0_40px_100px_rgba(26,22,16,0.08)] overflow-hidden group">
            ...
            <div className="relative z-10 text-center flex flex-col items-center">
              <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4 mb-10">
                <div className="h-px w-10 sm:w-16 bg-gradient-to-r from-transparent via-gold-400 to-transparent"></div>
                <p className="px-1 text-[10px] sm:text-[11px] tracking-[0.28em] sm:tracking-[0.5em] uppercase text-[#9e7425] font-semibold whitespace-nowrap">Premium Partnership</p>
                <div className="h-px w-10 sm:w-16 bg-gradient-to-r from-transparent via-gold-400 to-transparent"></div>
              </div>
