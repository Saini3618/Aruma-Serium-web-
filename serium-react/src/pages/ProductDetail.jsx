      ...
      <div className="min-h-screen bg-[#FBF9F4] overflow-hidden pt-24 md:pt-32">
            ...
            <div className="flex-1 flex flex-col justify-center reveal-up" style={{ animationDelay: '0.2s' }}>
              <div className="info-glass-card p-10 md:p-14 bg-white/40 backdrop-blur-3xl border border-white/60 rounded-[40px] shadow-[0_40px_100px_rgba(26,22,16,0.05)] relative overflow-hidden transition-all duration-500">
                ...
                <h1 className="font-serif text-5xl md:text-7xl text-[#1a1610] leading-tight mb-6">
                  {product.name.split(' ').slice(0, -1).join(' ')} <span className="italic text-gold-400">{product.name.split(' ').pop()}</span>
                </h1>
                ...
                {/* Tabs */}
                <div className="flex gap-10 mb-8 border-b border-black/5 pb-4">
                  {['description', 'notes'].map(tab => (
                    <button 
                      key={tab}
                      onClick={() => setActiveTab(tab)}
                      className={`text-[10px] tracking-[0.4em] uppercase font-black transition-all ${activeTab === tab ? 'text-gold-600 border-b-2 border-gold-400' : 'text-black/30 hover:text-black/50'} pb-4 -mb-[18px]`}
                    >
                      {tab}
                    </button>
                  ))}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {product.notes.map((note, i) => (
                        <div key={i} className="flex items-center gap-3">
                          ...
                          <span className="text-[12px] tracking-widest text-[#1a1610]/70 uppercase">{note}</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="flex flex-col items-center gap-10">
                  <a 
                    href={`https://wa.me/8302607949?text=${product.waText}`}
                    target="_blank" 
        ...
        <section className="py-24 bg-white/30 backdrop-blur-xl border-t border-black/5">
          <div className="max-w-screen-xl mx-auto px-8 grid grid-cols-1 md:grid-cols-3 gap-16">
            {[
              { icon: '✦', title: 'IFRA Certified', desc: 'Absolute safety and global standards in every drop.' },
        ...
        <div className="py-24 bg-[#1a1610] text-center relative overflow-hidden">
           ...
           <div className="relative z-10 max-w-2xl mx-auto px-8 reveal-up">
              <h2 className="font-serif text-4xl md:text-6xl text-white mb-10 leading-tight">Explore More <br/><span className="italic gold-shimmer-text">Extraordinary</span> Aromas</h2>
              <Link to="/" className="btn-gold px-12 py-5">View Full Collection</Link>
           </div>
        </div>
      </div>
    </Layout>
